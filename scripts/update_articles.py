...
ARTICLE_SCHEMA: Dict[str, Any] = {
    ...
    "properties": {
        ...
        "blurb": {
            "type": "string",
            "description": "Two to three simple but specific card sentences. Include what the researchers studied or used and the main result or claim. Avoid jargon and avoid vague implications."
        },
        "curiosityHook": {
            "type": "string",
        ...
        "summary": {
            "type": "string",
            "description": "A plain-English abstract in 8-12 short sentences. It should explain the exact research question, study type, what they used/did, what they found, and what is still uncertain."
        },
        "researchQuestion": {
            "type": "string",
        ...
        "deepDive": {
            "type": "string",
            "description": "A simple but meaningful science explanation in 3-5 short paragraphs. Explain the key mechanism, method, or idea without copying the abstract."
        },
        "whyItMatters": {
            "type": "string",
...
The goal is ScienceDaily-style clarity, but more student-friendly:
- simple words,
- short sentences,
- real study details,
- no academic fog,
- no generic motivational science language.

Every answer must stay focused on the exact paper provided. The reader should understand:
- what the paper was asking,
- what kind of study or paper it is,
- what the researchers used, built, measured, compared, reviewed, or analyzed,
- what they found or argued,
- what the result could specifically lead to,
- what the paper still does not prove.

Simple does NOT mean vague.
Simple means a normal person can understand the actual study without needing a research background.

Never write filler. Never copy the abstract. Never make the paper sound more proven than it is.
"""
...
1. the actual thing studied, built, measured, or tested,
2. a cautious verb like could, may, might, or suggests when the work is early,
3. the specific result or real-world possibility.

It must be catchy, but it must still clearly belong to THIS paper.
...
- 16 words maximum.
- Use the specific object of the paper: rats, EEG, printed muscle, older adults, tumor cells, bioinks, survey data, etc.
- Avoid vague words like "things," "ideas," "possibilities," "problems," or "function" when a specific noun is available.
...
The user wants writing that is simple, specific, and still detailed enough to teach.
...
1. Paper type: experiment, clinical trial, animal study, cell study, AI/data study, engineering study, review, protocol, framework, commentary, etc.
2. Research question: what exact problem did the paper focus on?
...
6. Specific implication: if this work keeps improving, what could it specifically help people do?
7. Limit: what does it NOT prove yet?

Every major field should include at least two concrete anchors from the actual paper, such as:
- the study subject or data source,
- the technology or material used,
- the disease, body system, environmental issue, or population,
- the measured outcome,
- the main comparison or result.
...
- This could advance regenerative medicine.
- This could help medicine and technology.

Do not copy the abstract's technical wording. Translate it.
...
- "tumor microenvironment" → "the cells and signals around a tumor that can affect how cancer behaves"

Length targets:
- blurb: 2–3 sentences, 70–110 words total. It should include the study focus, what was done, and the main result or claim.
- curiosityHook: 1 sentence, 15–30 words. Make the reader want to know what happened in this paper.
- summary: 8–12 short sentences, 220–330 words total. This is the plain-English abstract.
- researchQuestion: 1–2 simple sentences.
- whatTheyDid: 130–220 words.
- whatTheyFound: 160–260 words.
- mainResult: 1 simple sentence, specific to the paper.
- whatThisCouldLeadTo: 130–220 words. Be specific about possible future use.
- deepDive: 240–380 words. Explain the science simply, not exhaustively.
- whyItMatters: 3–4 specific sentences.
- limitations: 90–160 words.

FIELD STYLE:

blurb:
Keep it hooky and specific. Mention what the researchers actually studied or used, then say the main result in everyday language.
Do NOT use the blurb for vague future promises.
...
- What they did.
- What they found or argued.
- What the result could specifically help with later.
...
Bad: "This could advance regenerative medicine and biohybrid robotics."
Better: "If scientists can make printed muscle contract in a controlled way, they could build small muscle models for testing injury treatments, study muscle diseases in the lab, or create tiny living machines that move without metal motors. That is still far from repairing a patient's muscle."

whyItMatters:
Do not just name a field. Explain what the work could let researchers actually do.
For example: better drug testing, safer implants, earlier diagnosis, cleaner city policy, a better way to study disease, or a clearer way to design future experiments.

limitations:
Be specific but simple. Say exactly why the reader should be cautious.
...
def validate_summary_quality(summary: Dict[str, Any], raw: Dict[str, Any]) -> List[str]:
    ...
    if len(clean_text(summary.get("blurb"))) < 120:
        problems.append("Blurb is too short; it needs the actual study focus and result in simple language.")
    if len(clean_text(summary.get("summary"))) < 700:
        problems.append("Summary is too short or generic; it needs a plain-English abstract with real study details.")
    if len(clean_text(summary.get("whatTheyDid"))) < 450:
        problems.append("whatTheyDid is too short; it needs concrete method/material details from the abstract.")
    if len(clean_text(summary.get("whatTheyFound"))) < 500:
        problems.append("whatTheyFound is too short; it needs concrete results from the abstract.")
    if len(clean_text(summary.get("whatThisCouldLeadTo"))) < 400:
        problems.append("whatThisCouldLeadTo is too short or generic; it needs a specific future use/implication.")
    if len(clean_text(summary.get("deepDive"))) < 800:
        problems.append("deepDive is too short; it needs a simple but useful science explanation.")
